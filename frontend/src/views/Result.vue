<template>
  <div class="result-container">
      ...
      <a-space size="middle">
        ...
        <a-dropdown v-if="!editMode">
          ...
          <a-button type="default">
            <DownloadOutlined /> 导出行程 <DownOutlined />
          </a-button>
        </a-dropdown>
      </a-space>
    </div>
    ...
    <div v-if="tripPlan" class="content-wrapper">
        ...
        <a-affix :offset-top="80">
          <a-menu mode="inline" :selected-keys="[activeSection]" @click="scrollToSection">
            <a-menu-item key="overview">
              <span><UnorderedListOutlined style="margin-right: 6px" />行程概览</span>
            </a-menu-item>
            <a-menu-item key="budget" v-if="tripPlan.budget">
              <span><WalletOutlined style="margin-right: 6px" />预算明细</span>
            </a-menu-item>
            <a-menu-item key="map">
              <span><EnvironmentOutlined style="margin-right: 6px" />景点地图</span>
            </a-menu-item>
            <a-sub-menu key="days">
              <template #title><CalendarOutlined style="margin-right: 6px" />每日行程</template>
            ...
            <a-menu-item key="weather" v-if="tripPlan.weather_info && tripPlan.weather_info.length > 0">
              <span><CloudOutlined style="margin-right: 6px" />天气信息</span>
            </a-menu-item>
          </a-menu>
        </a-affix>
      </div>
      ...
      <div class="main-content">
        ...
        <div class="top-info-section">
          ...
          <div class="left-info">
            ...
            <a-card id="overview" :title="`${tripPlan.city}旅行计划`" :bordered="false" class="overview-card">
              <div class="overview-content">
                <div class="info-item">
                  <span class="info-label"><CalendarOutlined style="margin-right: 4px" />日期:</span>
                  <span class="info-value">{{ tripPlan.start_date }} 至 {{ tripPlan.end_date }}</span>
                </div>
                <div class="info-item">
                  <span class="info-label"><BulbOutlined style="margin-right: 4px" />建议:</span>
            ...
            <a-card id="budget" v-if="tripPlan.budget" :bordered="false" class="budget-card">
              <template #title><WalletOutlined style="margin-right: 8px" />预算明细</template>
          ...
          <div class="right-map">
            <a-card id="map" :bordered="false" class="map-card">
              <template #title><EnvironmentOutlined style="margin-right: 8px" />景点地图</template>
        ...
        <a-card :bordered="false" class="days-card">
          <template #title><CalendarOutlined style="margin-right: 8px" />每日行程</template>
          <a-collapse v-model:activeKey="activeDays" accordion>
            ...
            >
              ...
              <div class="day-info">
                <div class="info-row">
                  <span class="label"><FileTextOutlined style="margin-right: 4px" />行程描述:</span>
                  <span class="value">{{ day.description }}</span>
                </div>
                <div class="info-row">
                  <span class="label"><CarOutlined style="margin-right: 4px" />交通方式:</span>
                  <span class="value">{{ day.transportation }}</span>
                </div>
                <div class="info-row">
                  <span class="label"><HomeOutlined style="margin-right: 4px" />住宿:</span>
                  <span class="value">{{ day.accommodation }}</span>
                </div>
              </div>

              <!-- 景点安排 -->
              <a-divider orientation="left"><EnvironmentOutlined style="margin-right: 6px" />景点安排</a-divider>
              <a-list
              ...
              <!-- 酒店推荐 -->
              <a-divider v-if="day.hotel" orientation="left"><HomeOutlined style="margin-right: 6px" />住宿推荐</a-divider>
              ...
              <!-- 餐饮安排 -->
              <a-divider orientation="left"><RestOutlined style="margin-right: 6px" />餐饮安排</a-divider>
        ...
        <a-card id="weather" v-if="tripPlan.weather_info && tripPlan.weather_info.length > 0" style="margin-top: 20px" :bordered="false">
          <template #title><CloudOutlined style="margin-right: 8px" />天气信息</template>
        <a-list
    ...
    <a-empty v-else description="没有找到旅行计划数据">
      <template #image>
        <CompassOutlined style="font-size: 72px; color: #94a3b8;" />
      </template>
      <template #description>
...
// ============================================================
// 结果页组件(Result.vue):展示生成好的旅行计划。
// 从 sessionStorage 读取首页存进来的计划数据,然后:
// - 展示每日行程、预算、天气、酒店、餐饮;
// - 用高德地图 JS API 把景点标在地图上并连线;
// - 支持编辑(改地址/时长/删除/排序景点)和导出(图片/PDF)。
// ============================================================

import { ref, onMounted, nextTick } from 'vue'   // onMounted=组件加载后执行;nextTick=等 DOM 更新后再执行
...
// 一系列响应式状态
const tripPlan = ref<TripPlan | null>(null)           // 旅行计划数据(核心数据,可能为空)
const editMode = ref(false)                           // 是否处于"编辑模式"
const originalPlan = ref<TripPlan | null>(null)       // 进入编辑前的原始数据(取消编辑时恢复用)
const attractionPhotos = ref<Record<string, string>>({})  // 景点名称 -> 图片URL 的映射(缓存已加载的图)
const activeSection = ref('overview')                 // 侧边导航当前高亮的分区
...
const getAttractionImage = (name: string, index: number): string => {
  ...
  // 返回一个纯色占位图(避免跨域问题)
  const colors = [
    { start: '#0ea5a4', end: '#10b981' },
    { start: '#14b8a6', end: '#34d399' },
    { start: '#0d9488', end: '#5eead4' },
    { start: '#06b6d4', end: '#22d3ee' },
    { start: '#10b981', end: '#a7f3d0' }
  ]
...
const exportAsImage = async () => {
  try {
    ...
    exportContainer.style.backgroundColor = '#f0fdfa'
    ...
    cardHeads.forEach((head) => {
      ...
      try {
        headEl.style.setProperty('background-color', '#0ea5a4')
    ...
    hotelCards.forEach((card) => {
      ...
      if (head) {
        head.style.setProperty('background-color', '#0d9488')
      }
      (card as HTMLElement).style.setProperty('background-color', '#ccfbf1')
    })
    ...
    if (budgetTotal) {
      ...
      el.style.setProperty('background-color', '#0ea5a4')
    ...
    budgetItems.forEach((item) => {
      ...
      el.style.setProperty('background-color', '#f0fdfa')
...
// 初始化地图:加载高德地图 JS API,创建地图实例,并把景点标上去
const initMap = async () => {
  try {
    // 动态加载高德地图 JS API(要传 Web 端 JS Key)
    const AMap = await AMapLoader.load({
      key: import.meta.env.VITE_AMAP_WEB_JS_KEY,  // 高德地图Web端(JS API) Key(在 .env 里配)
      version: '2.0',
      // 要用到的插件:标记、折线、信息窗口
...
// 添加景点标记:把计划里的每个景点变成地图上的一个点,点一下弹出详情
const addAttractionMarkers = (AMap: any) => {
  if (!tripPlan.value) return

  const markers: any[] = []        // 存放所有标记对象
  const allAttractions: any[] = [] // 存放所有"有坐标"的景点

  // 遍历每一天、每个景点,把有经纬度的收集起来
  tripPlan.value.days.forEach((day, dayIndex) => {
    day.attractions.forEach((attraction, attrIndex) => {
      ...
      if (attraction.location && attraction.location.longitude && attraction.location.latitude) {
        allAttractions.push({
          ...attraction,   // 展开景点所有字段
          dayIndex,        // 记录它是第几天的(用于路线分组和提示)
          attrIndex        // 记录它是当天第几个景点
        })
      }
    })
  })
  ...
  allAttractions.forEach((attraction, index) => {
    const marker = new AMap.Marker({
      position: [attraction.location.longitude, attraction.location.latitude],  // 注意高德坐标顺序是 [经度, 纬度]
      title: attraction.name,   // 鼠标悬停显示的名称
      label: {
        // 标记上显示的数字编号(1、2、3...)
        content: `<div style="background: #4CAF50; color: white; padding: 4px 8px; border-radius: 4px; font-size: 12px;">${index + 1}</div>`,
        offset: new AMap.Pixel(0, -30)  // 让标签显示在标记上方
      }
    })
    ...
    const infoWindow = new AMap.InfoWindow({
      content: `
        <div style="padding: 10px;">
          ...
          <p style="margin: 4px 0; color: #0ea5a4;"><strong>第${attraction.dayIndex + 1}天 景点${attraction.attrIndex + 1}</strong></p>
        </div>
      `,
...
// 绘制路线:把每天的景点按顺序用折线连起来,示意游览顺序
const drawRoutes = (AMap: any, attractions: any[]) => {
  // 少于 2 个点画不成线,直接返回
  if (attractions.length < 2) return

  // 按"天"分组(每天的路线单独画,避免跨天乱连)
  const dayGroups: any = {}
  ...
  Object.values(dayGroups).forEach((dayAttractions: any) => {
    ...
    const polyline = new AMap.Polyline({
      path: path,                    // 经过的点
      strokeColor: '#0ea5a4',        // 线条颜色(蓝色)
      strokeWeight: 4,               // 线条粗细
      strokeOpacity: 0.8,            // 透明度
      strokeStyle: 'solid',          // 实线
      showDir: true                  // 显示方向箭头(指示游览方向)
    })
